...
class UNet(nn.Module):
    def __init__(self, config=None):
        ...
        # Decoder - transposed
        self.dconv1 = nn.ConvTranspose2d(features * 8, features * 4, kernel_size=2, stride=2, padding=0)
        self.dconv2 = nn.ConvTranspose2d(features * 4, features * 2, kernel_size=2, stride=2, padding=0)
        self.dconv3 = nn.ConvTranspose2d(features * 2, features, kernel_size=2, stride=2, padding=0)

        # Decoder - residual
        self.dres1 = ResidualBlock(features * 8, features * 4, dropout)  # After concatenation
        self.dres2 = ResidualBlock(features * 4, features * 2, dropout)  # After concatenation
        self.dres3 = ResidualBlock(features * 2, features, dropout)      # After concatenation
...
def visualise_pred(model_path=None, data_dir='_data/train/', n_samples=3, image_ids=None):
    ...
    plt.figure(figsize=(6*cols, 4*rows*2))
